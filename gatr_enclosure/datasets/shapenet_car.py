...
class ShapenetCarDataset(HierachicalDataFormatDatasetMemory):
    ...
    def hdf5_to_pyg(data_hdf5: h5py.Group) -> pyg.data.Data:
        ...
        pos_dirichlet_boundary = torch.from_numpy(data_hdf5["pos"][()])
...
class ShapenetCarDatasetUPT(HierachicalDataFormatDatasetMemory):
    ...
    def __init__(
        ...
        if hasattr(self.data, "eigenvalues"):
            ...
            print(
                f"Min eig: {me.min().item()} - {me.mean().item()} - {me.max().item()}"
            )
            print(
                f"Max eig: {Me.min().item()} - {Me.mean().item()} - {Me.max().item()}"
            )
    ...
    def hdf5_to_pyg(data_hdf5: h5py.Group) -> pyg.data.Data:
        ...
        dirichlet_boundary_normal = torch.from_numpy(
            data_hdf5["car_surface_normal"][()]
        )
        ...
        sdf = torch.cat(
            (
                torch.zeros(pos_dirichlet_boundary.size(0), dtype=sdf.dtype),
                sdf.reshape(-1),
            )
        )
        ...
        field = torch.cat(
            (
                dirichlet_boundary_normal,
                direction_average_velocity.expand(pos_domain.size(0), -1),
            )
        )
        dirichlet_boundary_idcs = torch.arange(
            pos_dirichlet_boundary.size(0), dtype=torch.int
        )
