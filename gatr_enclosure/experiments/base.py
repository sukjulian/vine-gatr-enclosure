...
class BaseExperiment(ABC):
    def __init__(self, config: DictConfig, checkpoint_dir: str = None, id: str = None, should_resume: bool = False):
        ...
        self._checkpoints_dir = os.path.join("checkpoints", self._id) if checkpoint_dir is None else os.path.join(checkpoint_dir, self._id)
    ...
    def _parse_config(config: DictConfig) -> DictConfig:
        config_ = cast(dict, OmegaConf.to_container(config, resolve=True, throw_on_missing=True))
    ...
    def _id(self) -> str:
        ...
        class_id = "-".join(re.findall(r"[A-Z][a-z]*|\d+", self.__class__.__name__)).lower()
    ...
    def train_model(self, device: torch.device) -> None:
        ...
        if self.config.training.lr_scheduler == 'exponential':
            lr_scheduler = ExponentialLR(optimiser, gamma=self.config.training.lr_decay_factor)
        elif self.config.training.lr_scheduler == 'plateau':
            lr_scheduler = ReduceLROnPlateau(optimiser, mode='min', patience=4, threshold=2e-2, factor=self.config.training.lr_decay_factor_plateau, min_lr=1e-5)
        else:
            raise ValueError()
        ...
        pbar_epochs = tqdm(
            range(starting_epoch, self.config.training.num_epochs), desc="Epochs", position=0, leave=True
        )

        if ("wandb" in self.config and self.config.wandb is True) or wandb.run is not None:
        ...
        for epoch in pbar_epochs:
            ...
            for step_count, data in enumerate(pbar_training, start=1):
                ...
                if "grad_scaler" in locals():
                    total_loss, other_metrics = self._optimiser_step_automatic_mixed_precision(
                        optimiser, data, grad_scaler, step_count
                    )
                else:
                    total_loss, other_metrics = self._optimiser_step(optimiser, data, step_count)
            ...
            model_log_dict_train = ({
                f"train/{k}": v
                for k, v in self._model.get_log_dict().items()
            } if hasattr(self._model, "get_log_dict") else {})  # computed for last batch only
            ...
            with torch.no_grad():
                ...
                if validation_loader is not None:
                    for idx, data in enumerate(
                        ...
                        scatter_idcs.append(torch.tensor(idx, device=device).expand(y_.size(0)))

                    metric = self.metric_fn(
                        torch.cat(y), torch.cat(y_data), torch.cat(scatter_idcs), config=self.config
                    ).tolist()

                    model_log_dict = (
                        self._model.get_log_dict() if hasattr(self._model, "get_log_dict") else {}
    ...
    def test_model(self, device: torch.device) -> None:
        ...
        with torch.no_grad():
            ...
            for idx, data in enumerate(
                tqdm(self._get_data_loader("test"), desc="Test split", position=0, leave=False)
            ):
                data.to(device)

                y.append(self._model(data))
                y_data.append(data.y)
                scatter_idcs.append(torch.tensor(idx, device=device).expand(data.y.size(0)))
    ...
    def _get_optimiser(self) -> torch.optim.Optimizer:
        ...
        iterator = zip(
            (filter, filterfalse), (self.config.training.var_lr, self.config.training.lr)
        )
    ...
    def _optimiser_step(
        ...
        if "grad_clipping" in self.config.training and self.config.training.grad_clipping is True:
    ...
    def _compute_loss(self, data: pyg.data.Data) -> Tuple[torch.Tensor, Dict, torch.Tensor]:
    ...
    def _save_best_model(self, model: torch.nn.Module) -> None:
        ...
        torch.save(model.state_dict(), os.path.join(self._checkpoints_dir, "best_model_checkpoint.pt"))

    def _load_best_model(self, model: torch.nn.Module) -> None:
        checkpoint_path = os.path.join(self._checkpoints_dir, "best_model_checkpoint.pt")
        if os.path.isfile(checkpoint_path):
            print(f'Loading best checkpoint from {checkpoint_path}')
            model.load_state_dict(torch.load(checkpoint_path))
        else:
            print('Checkpoint file not found, cannot load best checkpoint. Keeping current weights')
            # raise FileNotFoundError

    def _save_checkpoints(self, epoch: int, model: torch.nn.Module, optimiser: torch.optim.Optimizer, scheduler) -> None:

        self._create_checkpoints_dir()

        torch.save(model.state_dict(), os.path.join(self._checkpoints_dir, "model_checkpoint.pt"))
        torch.save(optimiser.state_dict(), os.path.join(self._checkpoints_dir, "optimiser_checkpoint.pt"))
    ...
    def _load_checkpoints(self, model: torch.nn.Module, optimiser: torch.optim.Optimizer, scheduler = None) -> int:

        path_model_checkpoint = os.path.join(self._checkpoints_dir, "model_checkpoint.pt")
        path_optimiser_checkpoint = os.path.join(self._checkpoints_dir, "optimiser_checkpoint.pt")
        checkpoint_path = os.path.join(self._checkpoints_dir, "current_checkpoint.h5")
    ...
    @staticmethod
    def _log_to_wandb(loss_cache: Dict[str, Any], epoch) -> None:
    ...
    def _save_random_visualisation(self) -> None:
        ...
        for key, value in {"y_model": y, **data.to_dict()}.items():
            ...
            elif "decoder_query_idcs" in locals() and value.size(0) == decoder_query_idcs.numel():
                dummy = torch.full((pos.size(0), *value.shape[1:]), value.min(), dtype=value.dtype)
        ...
        if "face" in data:
            ...
            point_cloud = point_cloud.compute_normals()  # enables smooth rendering in ParaView
